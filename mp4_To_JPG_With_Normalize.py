...
def getFrame(vid,folderPath,filename,sec,count):
    ...
    if hasFrames:
        cv2.imwrite(folderPath+"\\"+filename+str(count)+".jpg", image)
        try:
            img_array = cv2.imread(folderPath+"\\"+filename+str(count)+".jpg", cv2.IMREAD_GRAYSCALE)                
            new_img = cv2.resize(img_array, (SIZE,SIZE))
            cv2.imwrite(folderPath+"\\"+filename+str(count)+".jpg", new_img)
...
def convert(fp,fileName,end,videos,pictures):
    sec = 0
    frameRate = 0.01       #Make number number lower to capture more frames, right not it is at one frame every half a second
    count=1
